...
describe('[TIMESTAMP] diffTimestamp', () => {
  ...
  it('diffTimestamp includes seconds and milliseconds', () => {
    const ts = timestamp.parsed('2026-06-08T09:30:15.125')
    const ts2 = timestamp.parsed('2026-06-08T09:30:16.250')
    const tests = timestamp.diffTimestamp(ts, ts2)
    expect(tests).toBe(1125)
  })
})
