...
describe("[TIMESTAMP] parseTimestamp", () => {
  ...
  it("parseTimestamp keeps optional seconds and milliseconds", async () => {
    const tests = timestamp.parseTimestamp("2026-06-08T09:30:15.250Z");
    expect(tests.hasDay).toBe(true);
    expect(tests.hasTime).toBe(true);
    ...
    expect(tests.time).toBe("09:30:15.250");
  });
});
